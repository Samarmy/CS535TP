...
object KeepOne {
    ...
    def main(args: Array[String]) {
        ...
        var degreeGraph = graph.outerJoinVertices(graph.outDegrees)((id, oldAttr, outDegOpt) => (outDegOpt.getOrElse(0), false, oldAttr._3))
        ...
        //Pick important vertices. Vertices with degree greater than 5 (should change depending on dataset)
        val importantVertices = degreeGraph.vertices.filter{
          case (id, x) =>  x._1 > 5
        }
        ...
        val k2Edges = k1.edges.flatMap(x => {
        ...
        val totalVertices = Graph(k1.vertices, k2Edges, (0, true, Array.fill[Long](numLevels)(-1L))).vertices.map(x => {
          if(x._2._3(0) == -1L){
        ...
        val totalGraph = Graph(totalVertices, degreeGraph.edges).cache()
        ...
        val finalGraph = Graph(pregelGraph.vertices.mapValues(x => x._3) ++ degreeZeroVertices.mapValues(x => x._3), pregelGraph.edges)
